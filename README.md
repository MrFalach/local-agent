...
פועל עם קלוד קוד ומנתב אוטומטית כל בקשה — משימות קוד פשוטות הולכות למודל מקומי על המחשב שלך (חינם), משימות מורכבות הולכות לקלוד בענן לחשיבה בלבד — והביצוע עצמו מתבצע בחינם על המקומי.
...
**ענן** — ארכיטקטורה, דיבוג עמוק, אבטחה, שאלות "למה" → רק החשיבה עולה כסף

> **חשוב להבין:** גם כשבקשה מורכבת הולכת לענן — רק ה*חשיבה* מתבצעת שם.
> ה*כתיבה בפועל* של הקוד לקבצים מתבצעת תמיד מקומית, בחינם.
> כלומר: קלוד חושב בענן → מודל מקומי מייצר את הקוד → קלוד כותב לקבצים.
> עיקר העלות (ייצור טוקנים רב) נחסכת.
